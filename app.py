...
def ucitaj_podatke():
    ...
    tablica = klijent.open("FILMOVI") #otvara tablicu
    radni_list = tablica.worksheet("filmovi") #otvara radni list
...
if not filmovi.empty:
    filmovi["Godina"] = pd.to_numeric(filmovi["Godina"], errors="coerce")
    filmovi["Ocjena"] = pd.to_numeric(filmovi["Ocjena"], errors="coerce")
...
if filmovi.empty:
    ...
    if st.button("IZBRIŠI FILM"):
        if odabrani_indeks is not None:
            ...
            radni_list.delete_row(redak_u_tablici)
